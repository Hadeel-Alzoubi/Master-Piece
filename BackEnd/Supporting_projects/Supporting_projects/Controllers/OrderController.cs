...
namespace Supporting_projects.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public IActionResult CreateOrder([FromBody] OrdersDTO orderDto)
        {
            ...
            var u = _db.Carts.Where(x => x.UserId == orderDto.UserId).Select(x => new
            {
                x.CartId
            }).FirstOrDefault();
            ...
            var cartItems = _db.CartItems.Where(l => l.CartId == cartId).ToList();
                ...
                foreach (var cartItem in cartItems)
                {
                    var item = new OrderDetail()
                    {
                        ...
                        ProductIdFk = cartItem.ProductId,
        ...
        public IActionResult getorederDetails(int id)
        {
            var orderDetails = _db.OrderDetails
            .Where(od => od.OrderIdFk == id)
            .Join(_db.Products,
                  od => od.ProductIdFk,
                  p => p.ProductId,
                  (od, p) => new OrderWithDetailsDTO
                  {
                      OrderId = od.OrderId,
